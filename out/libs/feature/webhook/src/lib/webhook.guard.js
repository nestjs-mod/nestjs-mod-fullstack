...
let WebhookGuard = (WebhookGuard_1 = class WebhookGuard {
  ...
  throwErrorIfCurrentUserNotHaveNeededRoles(checkWebhookRole, req) {
    if (
      ...
      throw new webhook_errors_1.WebhookError(
        webhook_errors_1.WebhookErrorEnum.FORBIDDEN
      );
    }
  }
  throwErrorIfCurrentUserNotSet(req) {
    if (!req.webhookUser) {
      throw new webhook_errors_1.WebhookError(
        webhook_errors_1.WebhookErrorEnum.USER_NOT_FOUND
      );
    }
  }
  async tryGetOrCreateCurrentUserWithExternalUserId(
    req,
    externalTenantId,
    externalUserId
  ) {
    if (!req.webhookUser) {
      if (
        !externalTenantId ||
        !(0, class_validator_1.isUUID)(externalTenantId)
      ) {
        throw new webhook_errors_1.WebhookError(
          webhook_errors_1.WebhookErrorEnum.EXTERNAL_TENANT_ID_NOT_SET
        );
      }
  ...
  getExternalUserIdFromRequest(req) {
    ...
    if (
      !req.externalUserId ||
      !(0, class_validator_1.isUUID)(req.externalUserId)
    ) {
      throw new webhook_errors_1.WebhookError(
        webhook_errors_1.WebhookErrorEnum.EXTERNAL_USER_ID_NOT_SET
      );
    }
  ...
  getHandlersReflectMetadata(context) {
    const skipWebhookGuard =
      (typeof context.getHandler === 'function' &&
        this.reflector.get(
          webhook_decorators_1.SkipWebhookGuard,
          context.getHandler()
        )) ||
    ...
    const checkWebhookRole =
      (typeof context.getHandler === 'function' &&
        this.reflector.get(
          webhook_decorators_1.CheckWebhookRole,
          context.getHandler()
        )) ||
      (typeof context.getClass === 'function' &&
        this.reflector.get(
