...
|`schemaFile`|**optional**|```/usr/src/app/apps/server/src/prisma/app-schema.prisma```|
|`addMigrationScripts`|**optional**|```false```|
|`binaryTargets`|**optional**|[ ```native```, ```rhel-openssl-3.0.x```, ```linux-musl-openssl-3.0.x``` ]|
...
| ------ | ----------- | ----- |
|`featureName`|**optional**|```app```|
|`migrationsFolder`|**isNotEmpty** (migrationsFolder should not be empty)|```/usr/src/app/apps/server/src/migrations```|
...
|`authorizerUrl`|`obj['authorizerUrl']`, `process.env['SERVER_AUTHORIZER_AUTHORIZER_URL']`|**optional**|-|
